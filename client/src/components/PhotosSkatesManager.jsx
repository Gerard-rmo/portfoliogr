...
const PhotosSkatesManager = () => {
  ...
  const fetchPhotos = async () => {
    try {
      const res = await axiosConfig.get('/photos?categorie=skates');
      setPhotos(Array.isArray(res.data) ? res.data : res.data.photos || []);
    } catch (err) {
      console.error("Erreur récupération photos skates:", err);
    }
  };
      ...
      <div style={styles.photoGrid}>
        {photos.map(photo => (
          <div key={photo._id} style={styles.photoItem}>
            <img src={photo.imageURL} alt="Skate" style={styles.image} />
